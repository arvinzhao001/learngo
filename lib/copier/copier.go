...
func TestCopier() {
	var (
		...
		employees = []Employee{}
		oldUser   = User{Role: "Staff", Age: 0}
		newUser   = User{Name: "Qiang", Age: 19}
	)
	...
	//ignore empty field
	copier.CopyWithOption(&oldUser, &newUser, copier.Option{IgnoreEmpty: true, DeepCopy: true})
	fmt.Printf("%#v \n", oldUser)
	//copier.Copy(&oldUser, &newUser)
	//fmt.Printf("%#v \n", oldUser)
}
